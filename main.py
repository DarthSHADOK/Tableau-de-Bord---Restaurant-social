...
class MainWindow(QMainWindow):
    ...
    def trigger_update_download(self, dialog_ref):
        # --- CORRECTION DE L'URL POUR LES BÊTAS ---
        # On ne peut pas utiliser "latest" car GitHub l'ignore pour les bêtas.
        # On utilise le tag exact détecté par l'UpdateWorker.
        
        tag = self.new_version_detected
        # On s'assure que le tag commence par 'v' pour l'URL GitHub
        if not tag.startswith("v"): 
            tag = f"v{tag}"
            
        print(f"Téléchargement de la version : {tag}") # Debug
        
        # URL spécifique au tag (marche pour Stable ET Bêta)
        url = f"https://github.com/DarthSHADOK/Tableau-de-Bord---Restaurant-social/releases/download/{tag}/GestionResto.zip"
        
        # Le reste ne change pas
        temp_path = os.path.join(tempfile.gettempdir(), "GestionResto.zip")
        ...
        self.dl_worker.error.connect(lambda err: QMessageBox.critical(self, "Erreur", f"Échec du téléchargement ({tag}):\n{err}"))
    ...
    def apply_update(self, downloaded_path):
        try:
            ...
            # 1. Identifier où est l'updater "caché" (Mode OneFile ou Normal)
            if hasattr(sys, '_MEIPASS'):
                # En mode OneFile, il est dans le dossier temporaire
                internal_updater = os.path.join(sys._MEIPASS, "Systeme", "updater.exe")
            else:
                # En mode développement, il est juste dans le dossier Systeme
                internal_updater = os.path.join(install_dir, "Systeme", "updater.exe")

            # 2. Définir où on va le copier pour l'exécuter (Dossier d'install)
            # On le sort pour qu'il soit indépendant
            target_updater = os.path.join(install_dir, "updater.exe")
            
            # 3. Extraction / Copie de l'updater
            if os.path.exists(internal_updater):
                try:
                    shutil.copy2(internal_updater, target_updater)
                except Exception as e:
                    print(f"Erreur copie updater: {e}")
            else:
                QMessageBox.critical(self, "Erreur", f"Updater introuvable dans le paquet !\n{internal_updater}")
                return

            # 4. Lancement de l'updater extrait
            if os.path.exists(target_updater):
                # On lance l'updater externe
                subprocess.Popen([target_updater, downloaded_path, install_dir, current_exe])
                QApplication.quit()
            else:
                QMessageBox.critical(self, "Erreur", "Impossible d'extraire le programme de mise à jour.")

        except Exception as e: 
            QMessageBox.critical(self, "Erreur", f"Erreur critique MAJ:\n{e}")
