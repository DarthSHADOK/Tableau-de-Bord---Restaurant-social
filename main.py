...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Tableau de bord - Restaurant Social v{APP_VERSION}")
    ...
    def trigger_update_download(self, dialog_ref):
        self.current_dialog_ref = dialog_ref 
        
        tag = self.new_version_detected
        if not tag.startswith("v"): 
            tag = f"v{tag}"
            
        print(f"Tentative Mise à jour vers : {tag}")
        
        # 1. ESSAI ZIP (PRIORITAIRE)
        # On utilise le tag exact, ce qui corrige le problème des Bêtas
        url_zip = f"https://github.com/DarthSHADOK/Tableau-de-Bord---Restaurant-social/releases/download/{tag}/GestionResto.zip"
        path_zip = os.path.join(tempfile.gettempdir(), "GestionResto.zip")
        
        print(f"  -> Essai ZIP: {url_zip}")
        self.dl_worker = DownloadWorker(url_zip, path_zip)
        self.dl_worker.progress.connect(dialog_ref.update_progress)
        self.dl_worker.finished.connect(self.confirm_install)
        # Si le ZIP échoue (404), on passe au Plan B
        self.dl_worker.error.connect(lambda e: self.fallback_to_exe(tag, e))
        self.dl_worker.start()

    def fallback_to_exe(self, tag, error_msg):
        print(f"  -> ZIP introuvable ({error_msg}). Tentative EXE...")
        
        # 2. ESSAI EXE (FALLBACK)
        url_exe = f"https://github.com/DarthSHADOK/Tableau-de-Bord---Restaurant-social/releases/download/{tag}/GestionResto.exe"
        path_exe = os.path.join(tempfile.gettempdir(), "GestionResto.exe")
        
        self.dl_worker = DownloadWorker(url_exe, path_exe)
        self.dl_worker.progress.connect(self.current_dialog_ref.update_progress)
        self.dl_worker.finished.connect(self.confirm_install)
        self.dl_worker.error.connect(lambda e: QMessageBox.critical(self, "Erreur Mise à jour", f"Impossible de télécharger la mise à jour (ni ZIP, ni EXE).\n\nErreur: {e}"))
        self.dl_worker.start()
    ...
    def apply_update(self, downloaded_path):
        try:
            ...
            # CAS 1 : DOSSIER (ZIP EXTRAIT)
            if os.path.isdir(downloaded_path):
                updater_src = os.path.join(downloaded_path, "Systeme", "updater.exe")
                if not os.path.exists(updater_src):
                    updater_src = os.path.join(install_dir, "Systeme", "updater.exe")
                
                if os.path.exists(updater_src):
                    # Méthode Pro : Copie de l'updater dans TEMP pour éviter le verrouillage
                    updater_tmp = os.path.join(tempfile.gettempdir(), f"updater_{int(time.time())}.exe")
                    shutil.copy2(updater_src, updater_tmp)
                    
                    # Lancement : Updater(Source, Cible, ExeARelancer)
                    subprocess.Popen([updater_tmp, downloaded_path, install_dir, current_exe])
                    QApplication.quit()
                    return

            # CAS 2 : FICHIER (EXE SEUL) OU PAS D'UPDATER
            batch_script = os.path.join(install_dir, "update.bat")
            
            if os.path.isdir(downloaded_path):
                # Dossier sans updater -> xcopy
                cmd = f"""@echo off
...
del "%~f0"
"""
            with open(batch_script, "w") as f: f.write(cmd)
            subprocess.Popen([batch_script], shell=True, creationflags=subprocess.CREATE_NO_WINDOW)
            QApplication.quit()
            
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Erreur installation : {e}")
